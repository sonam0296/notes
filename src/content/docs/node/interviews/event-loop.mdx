---
title: ⚡ Event Loop in Browser JS vs Node.js
---
...
### 🔹 Browser JavaScript (Chrome, Firefox, etc.)
...
- **Priority Rule**:
  - After each stack execution → run **all microtasks** → then move on to **one macrotask**.
...
| Special Queue        | ❌ None                          | ✅ `process.nextTick` (higher priority than microtasks) |
| Order of async tasks | Microtasks → Macrotasks          | `process.nextTick` → Microtasks → Macrotasks            |
...
- _"In Nodejs"_ : `process.nextTick` runs first, then `Promises` (microtasks), then `timers` and `setImmediate` depending on the phase.”
